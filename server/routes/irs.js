  ...
  try {
    ...
    const q = (corretora, categoria) => {
      const rows = db.prepare(`SELECT
        simbolo, data_abertura, data_fecho, pl_eur,
        valor_compra_eur, valor_venda_eur, pais, moeda_original, conta
        FROM trades
        WHERE strftime('%Y', data_fecho) = ?
          AND corretora = ? AND categoria = ?
        ORDER BY data_fecho ASC`).all(ano, corretora, categoria);
      return rows.map(r => ({
        ...r,
...
// GET /api/irs/export?ano=2025  — devolve ficheiro Excel
router.get("/export", async (req, res) => {
  try {
    ...
    const addSheet = (name, rows, cols) => {
      ...
      if (!rows.length) {
        ws.addRow(["Sem dados para este quadro."]);
        return;
      }
